...
/-! ## Closing fold after reinsertion -/

/-- The reinserted diagonal gap has the internal peak shape `(k + k) + 2`. -/
theorem collatzIntersectionDiagonalGap_eq_double_formed_add_two
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    collatzIntersectionDiagonalGap intersection =
      (formedPositiveExcessOfIntersection intersection +
        formedPositiveExcessOfIntersection intersection) + 2 := by
  rw [collatzIntersectionDiagonalGap_eq_indexGap intersection]
  rw [collatzDiagonalGap_eq_double_add_two]

/--
The dynamic consumer's closing action folds the reinserted diagonal gap by the
closing/forming action.  This exposes the internal fold as `gap / 2` without
using any classical parity code or audited Nat division cancellation theorem.
-/
...
/--
The dynamic consumer's closing action folds the reinserted diagonal gap to the
successor of the formed index.
-/
theorem collatzClosingActionOfDynamicConsumer_on_gap_eq_succ_formedIndex
    ...
    collatzClosingActionOfIntersection
        (collatzDynamicClosureLoop intersection).consumer
        (collatzIntersectionDiagonalGap intersection) =
      formedPositiveExcessOfIntersection intersection + 1 := by
  rw [collatzClosingActionOfDynamicConsumer_on_gap_eq_half_gap intersection]
  rw [collatzIntersectionDiagonalGap_eq_double_formed_add_two intersection]
  exact
    nat_double_add_two_div_two
      (formedPositiveExcessOfIntersection intersection)

/--
Package for the local closing fold after dynamic reinsertion.

It records the audit-clean parts of the mechanism: the consumer carries the
reinserted gap as `closingExcess`, that gap has the internal peak shape
`(k + k) + 2`, and its closing action folds that same gap first by `/ 2`,
then to the successor of the formed index.
-/
...
/-- The canonical closing reinsertion fold package for one Collatz intersection. -/
def collatzClosingReinsertionFold
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    CollatzClosingReinsertionFold intersection where
  consumer := (collatzDynamicClosureLoop intersection).consumer
  consumer_eq := rfl
  reinsertedGap := collatzIntersectionDiagonalGap intersection
  reinsertedGap_eq := rfl
  reenters_as_closing :=
    collatzIntersectionDiagonalGap_reenters_as_closing intersection
  reinsertedGap_eq_double_formed_add_two :=
    collatzIntersectionDiagonalGap_eq_double_formed_add_two intersection
  folds_to_half_gap :=
    collatzClosingActionOfDynamicConsumer_on_gap_eq_half_gap
      intersection
  folds_to_successor :=
    collatzClosingActionOfDynamicConsumer_on_gap_eq_succ_formedIndex
      intersection

/-- The canonical fold consumer carries the reinserted gap as `closingExcess`. -/
theorem collatzClosingReinsertionFold_reenters_as_closing
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    arithmeticClosingRoleOfIntersection
        (collatzClosingReinsertionFold intersection).consumer =
      NatEnrichedParityRole.closingExcess
        (collatzClosingReinsertionFold intersection).reinsertedGap :=
  (collatzClosingReinsertionFold intersection).reenters_as_closing

/-- The canonical fold exposes the internal peak shape of the reinserted gap. -/
theorem collatzClosingReinsertionFold_gap_eq_double_formed_add_two
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    (collatzClosingReinsertionFold intersection).reinsertedGap =
      (formedPositiveExcessOfIntersection intersection +
        formedPositiveExcessOfIntersection intersection) + 2 :=
  (collatzClosingReinsertionFold intersection).reinsertedGap_eq_double_formed_add_two

/-- The canonical fold maps the reinserted gap through the closing half-action. -/
theorem collatzClosingReinsertionFold_folds_to_half_gap
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    collatzClosingActionOfIntersection
        (collatzClosingReinsertionFold intersection).consumer
        (collatzClosingReinsertionFold intersection).reinsertedGap =
      (collatzClosingReinsertionFold intersection).reinsertedGap / 2 :=
  (collatzClosingReinsertionFold intersection).folds_to_half_gap

/-- The canonical fold maps the reinserted gap to the successor of the formed index. -/
theorem collatzClosingReinsertionFold_folds_to_successor
    {branch : MemoryBranch}
    (intersection : PrimitiveMemoryReadingIntersection branch) :
    collatzClosingActionOfIntersection
        (collatzClosingReinsertionFold intersection).consumer
        (collatzClosingReinsertionFold intersection).reinsertedGap =
      formedPositiveExcessOfIntersection intersection + 1 :=
  (collatzClosingReinsertionFold intersection).folds_to_successor
...
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzIntersectionDiagonalGap_eq_double_formed_add_two
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingActionOfDynamicConsumer_on_gap_eq_half_gap
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingActionOfDynamicConsumer_on_gap_eq_succ_formedIndex
#print axioms Meta.EnrichedNatClosedStabilityInstance.CollatzClosingReinsertionFold
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingReinsertionFold
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingReinsertionFold_reenters_as_closing
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingReinsertionFold_gap_eq_double_formed_add_two
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingReinsertionFold_folds_to_half_gap
#print axioms Meta.EnrichedNatClosedStabilityInstance.collatzClosingReinsertionFold_folds_to_successor
/- AXIOM_AUDIT_END -/
