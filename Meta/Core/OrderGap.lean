...
This file records the order-theoretic test for the gap framework.

An order on the visible referential can compare projected values.  A
structural gap shows that this comparison must not be confused with equality of
the formed interface: two separated interfaces can have the same visible
projection, hence are mutually comparable in any visible preorder.  Even a
visible partial order only identifies the projected values; it does not
identify the enriched interfaces.
-/
...
structure VisiblePreorder (Visible : Type v) : Type v where
  ...
  refl : ∀ visible : Visible, le visible visible
  trans :
...
def VisibleOrderEquivalent
    ...
    (left right : Visible) :
    Prop :=
  order.le left right ∧ order.le right left
...
theorem visibleOrderEquivalent_refl
  ...
  ⟨order.refl visible, order.refl visible⟩
