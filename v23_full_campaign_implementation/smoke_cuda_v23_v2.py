#!/usr/bin/env python3
"""Non-scientific CUDA smoke v2 with deterministic discrete sampling."""
...
from v23.training_cuda_smoke_v2 import train_one_cuda_smoke_v2
...
def _training_smoke(
    ...
    report = train_one_cuda_smoke_v2(config, SymbolicDomain())
    return {
        "checkpoint": report["final_checkpoint"],
        "checkpoint_path": str(output / report["final_checkpoint"]),
        "cuda_available": report["cuda_available"],
        "forbidden_bypass_violations": report["forbidden_bypass_violations"],
        "metrics": report["metrics"],
        "parameters": report["parameters"],
        "regime": regime,
        "sampling": report["cuda_sampling_v2"],
    }


def _state_dicts_equal(first_path: str, second_path: str) -> bool:
...
def run_cuda_smoke(output_directory: str, iterations: int) -> dict[str, object]:
    ...
    causal_replay = _training_smoke(
        output / "training_causal_replay_smoke", "R_causal", 24
    )
    causal_replay_equal = (
        causal["metrics"] == causal_replay["metrics"]
        and _state_dicts_equal(
            str(causal["checkpoint_path"]),
            str(causal_replay["checkpoint_path"]),
        )
    )
    torch.cuda.synchronize(0)
    properties = torch.cuda.get_device_properties(0)
    script_path = Path(__file__).resolve()
    adapter_path = Path(train_one_cuda_smoke_v2.__code__.co_filename).resolve()
    report = {
        "schema": "v23-cuda-smoke-2",
        "scientific_result": False,
        "ok": tensor_smoke_ok
        and bool(supervised["cuda_available"])
        and bool(causal["cuda_available"])
        and causal_replay_equal,
        "command": [str(script_path), *sys.argv[1:]],
        "script_sha256": sha256_file(script_path),
        "sampling_adapter_sha256": sha256_file(adapter_path),
        ...
        "training": {
            ...
            "causal_replay_bitwise_equal": causal_replay_equal,
        },
    }
