...
class MoleculeProfile:
    ...
    has_formally_charged_atom: bool
                ...
                if product_field.number == 1:
                    ...
                    if measurement_type == "YIELD":
                        ...
                        if len(percentage_fields) == 1:
                            ...
                            if any(
                                field.wire_type != 5 for field in numeric_envelopes
                            ):
                                raise ValueError(
                                    "valeur Percentage sans wire type float32"
                                )
...
def canonicalize_component(
    ...
    reaction_pb2: Any,
    chemistry: Any,
    molecule_cache: dict[str, tuple[str, MoleculeProfile]],
    ...
    if smiles_values[0] in molecule_cache:
        canonical, profile = molecule_cache[smiles_values[0]]
    ...
    has_formally_charged_atom = any(
        atom.GetFormalCharge() != 0 for atom in molecule.GetAtoms()
    )
    cw1_syntactic_compatible = (
        set(elements) <= {"C", "N", "O"}
        and total_formal_charge == 0
        and not has_formally_charged_atom
    ...
    profile = MoleculeProfile(
        ...
        has_formally_charged_atom=has_formally_charged_atom,
        ...
        cw1_syntactic_compatible=cw1_syntactic_compatible,
    )
    molecule_cache[smiles_values[0]] = (canonical, profile)
    return canonical, role, profile, sorted(identifiers)
...
def profile_record(
    ...
    properties = {
        ...
        "formally_charged_atom": sum(
            profile.has_formally_charged_atom for profile in selected
        ),
        "net_formal_charge_nonzero": sum(
    ...
    return {
        ...
        "cw1_syntactic_upper_bound_molecules": sum(
            profile.cw1_syntactic_compatible for profile in selected
        ),
    }
