...
def lean_ratio(value: dict[str, int], signed: bool = False) -> str:
    ...
    constructor = "ImportedSignedRatio.mk" if signed else "ImportedPositiveRatio.mk"
    return constructor + " " + rendered_numerator + " " + str(value["denominator"])


def lean_amount(value: dict[str, Any]) -> str:
    return (
        "ImportedAmount.mk (" + lean_ratio(value["value"])
        + ") ." + value["unit"]
    )
...
def render_component(component: dict[str, Any]) -> str:
    return (
        "ImportedComponent.mk ." + component["group"]
        + " " + str(component["species_index"])
        + " ." + component["role"]
        + " (" + lean_amount(component["amount"]) + ")"
    )
...
def render_condition(condition: dict[str, Any]) -> str:
    return (
        "ImportedPhysicalCondition.mk ("
        + lean_ratio(condition["temperature"], signed=True) + ")"
        + " ." + condition["temperature_unit"]
        + " ." + condition["temperature_control"]
        + " ." + condition["pressure_control"]
        + " ." + condition["atmosphere"]
        + " ." + condition["stirring"]
        + " " + str(condition["reflux"]).lower()
    )
...
def render_environment(environment: dict[str, Any], indent: str) -> str:
    ...
    return (
        "EnvironmentTemplate.mk \"" + environment["semantic_condition_sha256"] + "\""
        + " \"" + environment["canonical_content_sha256"] + "\""
        + " (" + lean_amount(environment["amine_amount"]) + ")"
        + " (" + lean_amount(environment["acid_amount"]) + ")"
        + " (" + auxiliaries + ")"
        + " (" + additions + ")"
        + " (" + render_condition(environment["condition"]) + ")"
    )
