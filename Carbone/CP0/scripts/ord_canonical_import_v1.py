...
def fragment_record(fragment: Any, chemistry: Any) -> dict[str, Any]:
    ...
    atoms = [atom_record(atom) for atom in canonical.GetAtoms()]
    bonds = sorted(
...
def render_atom(atom: dict[str, Any]) -> str:
    return (
        ...
        + ", formalCharge := " + lean_int(atom["formal_charge"])
        + ", implicitHydrogens := " + str(atom["implicit_hydrogens"])
        + ", aromatic := " + lean_bool(atom["aromatic"])
        + ", tetraStereo := ." + atom["tetra_stereo"] + " }"
    )
...
def render_species(species: dict[str, Any], indent: str) -> str:
    ...
    return (
        "{ identitySha256 := \"" + species["identity_sha256"] + "\""
        + "\n" + indent + "  canonicalGraphSha256 := \""
