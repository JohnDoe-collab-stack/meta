...
Version 2 replaces the target-dependent tie-break of B2 v1 with a deterministic
pair-identity tie-break. It also avoids calling Spearman on a constant vector.
"""
...
def b2_predictions(
    ...
    for row in evaluation:
        ...
        for k in K_VALUES:
            ...
            weights = [
                max(similarity_value, 1e-6) ** 2
                for similarity_value, _, _, _ in selected
            ]
            predictions[k].append(
                sum(
                    weight * target
                    for weight, (_, _, _, target) in zip(weights, selected)
                )
                / sum(weights)
            )
    return predictions
...
def metrics(observed: Any, predicted: Any, deps: dict[str, Any]) -> dict[str, float]:
    ...
    if np.all(observed == observed[0]) or np.all(clipped == clipped[0]):
        correlation = 0.0
    else:
